...
namespace DatingAppUaa.UnitTests.Pruebas
{
    public class AdminControllerTests
    {
        ...
        public async Task OK_GetUsersWithRoles(string statusCode, string username, string password)
        {
            // Arrange
        ...
        public async Task OK_EditRoles(string statusCode, string username, string password, string user2, string roles)
        {
            // Arrange
            var user = await LoginHelper.LoginUser(username, password);
            _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", user.Token);
        ...
        [Theory]
        [InlineData("OK", "admin", "Pa$$w0rd")]
        public async Task OK_GetPhotosForModeration(string statusCode, string username, string password)
        {
            // Arrange
            var user = await LoginHelper.LoginUser(username, password);
            _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", user.Token);
